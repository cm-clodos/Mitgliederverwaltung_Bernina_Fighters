...
describe("Test the member administration workflow", () => {
  ...
  it("the h1 contains the correct text", () => {
    cy.getByData("site-title").should("exist").contains("Mitglied hinzufügen");
  });
  ...
  context("Check the form error messages", () => {
    it("Add a member with empty fields, should display error messages", () => {
      ...
      cy.getByData("error-message-firstname").should("exist").contains("Vorname ist erforderlich");
      cy.getByData("error-message-lastname").should("exist").contains("Nachname ist erforderlich");
  ...
  context("Add a valid member", () => {
    it("should add a new member", () => {
      ...
      cy.get("tbody").then(($tbody) => {
        cy.wrap($tbody).find('td[data-cell="nachname"]').contains("Mustermann").should("exist");
      });
      cy.get(".v-toast").should("exist").contains("Mitglied erfolgreich hinzugefügt");
    });
  });
  context("Add member with existing email", () => {
    it("should display a notification, email already exist", () => {
      ...
      cy.getByData("input-entry-date").type("2020-01-01");
      cy.getByData("submit-btn").click();
      cy.get(".v-toast").should("exist").contains("Email oder Telefonnummer existiert bereits");
    });
  });
  ...
  context("Delete a member and confirm the confirm dialog", () => {
    it("Delete a member and confirm the confirm dialog should delete the member", () => {
      ...
      cy.get("tbody").then(($tbody) => {
        ...
        cy.get(".v-toast").should("exist").contains("Mitglied erfolgreich gelöscht");
        cy.logout();
        cy.reload();
      });
    });
