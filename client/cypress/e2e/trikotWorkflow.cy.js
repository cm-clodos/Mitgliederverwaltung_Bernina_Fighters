...
describe("Test the trikot administration workflow", () => {
  ...
  it("the h1 contains the correct text", () => {
    cy.getByData("site-title").should("exist").contains("Trikot hinzufügen");
  });
  ...
  context("Add a new trikot", () => {
    it("Add a valid trikot", () => {
      ...
      cy.get("tbody").then(($tbody) => {
        cy.wrap($tbody).find('td[data-cell="nummer"]').should("exist").contains("1");
        cy.get(".v-toast").should("exist").contains("Trikot erfolgreich hinzugefügt");
      });
    });
  });
  ...
  context("make the trikot not available and save", () => {
    it("change the trikot to not available should uncheck the checkbox", () => {
      ...
      cy.get(".v-toast").should("exist").contains("Trikot erfolgreich aktualisiert");
      cy.get("tbody").then(($tbody) => {
        cy.wrap($tbody).find('[data-test="checkbox-available"]').eq(0).should("not.be.checked");
      });
    });
  });
  ...
  context("Delete a trikot and confirm the confirm dialog", () => {
    it("Delete a trikot and confirm the confirm dialog should delete the trikot", () => {
      ...
      cy.get("tbody").then(($tbody) => {
        ...
        cy.get(".v-toast").should("exist").contains("Trikot erfolgreich gelöscht");
        cy.logout();
        cy.reload();
      });
    });
